...
function buildReviewPrompt(issueId: string): string {
  return [
    ...
    `2. Run git diff --name-status <base_commit> to list changed files`,
    `3. Exclude test files (*.test.*, *.spec.*, __tests__/, tests/, test/, *_test.*, *.snap)`,
    `4. Run git diff <base_commit> -- <non-test-files> to review the actual changes`,
    `5. Read issues/${issueId}/tech_design.md to understand the design`,
    `6. Read issues/${issueId}/bugs.md for existing bug history`,
    '',
    'For each pending_verification bug: verify the fix.',
    '  - If fixed correctly → update bugs.md to resolved. Write "Fix confirmed → resolved" in the current round\'s Fix entry.',
    '  - If still broken → update bugs.md to unresolved. Append a new Round to the bug\'s History section explaining what\'s wrong and a new fix suggestion.',
    '',
    'For new issues found: create a new bug<n>.md in issues/' + issueId + '/bugs/',
    'following the format in the orb-code-reviewer skill. Add the entry to bugs.md with status=unresolved.',
...
function buildFixPrompt(issueId: string, bugs: BugEntry[]): string {
  ...
  return [
    ...
    '',
    'For each bug:',
    `1. Read issues/${issueId}/bugs/bug<n>.md for the full history and latest fix suggestion`,
    `2. If false positive: write "False positive — [reason]" in the current round's Fix entry, mark as blocked in bugs.md`,
    `3. Otherwise, write your fix approach in the current round's Fix entry`,
    `4. Fix the code in the worktree`,
    `5. Append the result (→ pending_verification) to the Fix entry, then update bugs.md`,
    '',
    'If a bug cannot be fixed (needs design change):',
    '  - Write the reason in the current round\'s Fix entry',
    '  - Mark as blocked in bugs.md and fill the Block reason column',
    '',
